...
SELECT *
FROM sys.fn_get_audit_file ('/var/opt/mssql/*.sqlaudit',default,default)
...
(	FILEPATH = N'/var/opt/mssql/'
	...
	,MAX_FILES = 4
	,RESERVE_DISK_SPACE = OFF
	--, AUDIT_GUID = '27083d3e-db30-47a2-9d4c-c7e50b06ef35'
) WITH (QUEUE_DELAY = 1000, ON_FAILURE = CONTINUE)
WHERE ([session_server_principal_name]<>'ubuntusql1\ubuntusql1$'
		and schema_name <>'sys')
...
CREATE SERVER AUDIT [AuditSpecification_testingdb]
TO FILE 
(FILEPATH = N'/var/opt/mssql/data/audit/testing/'
,MAXSIZE = 10 MB
,MAX_FILES = 10
,RESERVE_DISK_SPACE = OFF
) WITH (QUEUE_DELAY = 1000, ON_FAILURE = CONTINUE)
